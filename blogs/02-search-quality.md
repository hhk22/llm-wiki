...
[별도 표현 6문항의 후보 재평가](https://github.com/hhk22/llm-wiki/blob/v2-search-quality/evaluation/results-v2-followups-validation.json)에서는 가중 hybrid가 **4/6 → 5/6**으로 올랐지만, 참조 보강은 점수를 바꾸지 못했다. 가이드가 Top 3의 마지막에 있거나 참조가 없으면 필요한 장애 원문을 추가하지 못한다. 문서 순위·참조 보강·답변 근거는 각각 평가해야 한다.
...
flowchart TD
    ...
    N --> K[키워드 검색]
    V --> T[문서 Top 3]
    K -. hybrid 옵션 .-> H[가중 RRF 결합]
    V -. hybrid 옵션 .-> H
    H --> T
    T --> L[정책 원인 질문: 참조 장애를 Top 3 안에 보강]
    L --> C[문서당 최대 2개 청크]
    C --> A[근거 번호를 붙인 답변 하나]
```
...
| 문서당 청크 1개 → 최대 2개 | 답변에 필요한 근거가 보존되는가 |
| 벡터 + 필터 → 원인 장애 참조 보강 | 필요한 장애 원문이 함께 검색되는가 |
| 동일 가중 RRF → 벡터 가중 RRF | Top 3 회복과 1위 퇴보가 함께 나타나는가 |

이후 Hybrid 비교에는 보정한 키워드를 사용한다. Hit@1은 단일 정답 15개, Hit@3는 전체 20개에 대해 v1과 같은 방식으로 계산한다. 교차 문서 질문은 필요한 두 문서가 모두 Top 3에 있어야 성공이다. 문서 적중 평가는 문서 ID의 중복을 제거하고, 답변에 전달한 청크의 근거 내용은 별도로 확인한다.
...
기존 20문항은 이미 실패를 살펴본 회귀 평가 세트다. 별도로 표현을 바꾼 6문항은 필터 구현 후 평가 실행 전에 고정했으며, 결과를 보고 규칙을 재조정하지 않았다. 개발자가 작성한 추가 검증이며 독립적인 벤치마크로 보지는 않는다. 미래 버전 선택은 DB 통합 테스트로 확인하고, 기존 20문항 점수와 분리해 보고한다.

## 완료 기준

이번 hybrid 실험은 `evaluation/results-v2-hybrid.json`에 기록했고 v1 결과는 보존했다. 필터 실험은 `evaluation/results-v2-filters.json`, 청크 수 비교와 실제 답변은 `evaluation/results-v2-evidence.json`에 기록했다. 이후 재정렬 실험도 설정과 결과를 보존하고 전후 수치와 대표 성공·실패 사례를 추가한다.

v2의 완료 기준은 **어떤 변경이 어떤 질문을 개선했고, 어떤 실패가 남았는지 재현 가능한 기록으로 설명하는 것**이다. 그 결과를 보고 Wiki 생성·문서 간 연결 같은 다음 단계가 필요한지 판단한다.

v1 구현은 [기준선 브랜치](https://github.com/hhk22/llm-wiki/tree/docs/llm-wiki-devlog), v2 실험 기록과 이후 작업은 [검색 품질 개선 브랜치](https://github.com/hhk22/llm-wiki/tree/v2-search-quality)에서 확인할 수 있다. 현재 답변 경로는 버전 필터·문서당 최대 2개 청크·정책 원인 참조를 사용한다. Hybrid 옵션에는 키워드 1 : 벡터 3의 RRF 가중치를 적용했다.
