...
**현재 문제:** “현재” 배포 방법을 물어도 최신 가이드가 검색되지 않았다. 1단계의 키워드 보정과 2단계의 RRF에는 문서가 최신 정책인지 판단하는 조건이 없었다.
...
**무엇이 잘못됐나?** 정답인 `deploy-guide-v30`이 양쪽 Top 3에 없다. 벡터 검색은 최신 정책 5문항 모두에서 최신 가이드를 Top 3에 넣지 못했다.

**왜?** 키워드는 단어 일치도, 벡터는 의미 유사도로 순위를 정할 뿐 버전을 비교하지 않는다. RRF도 이 순위를 합산하므로 최신성을 보장하지 못한다.

**어떻게 개선했나? — 질문 의도로 검색 범위를 먼저 정한다.**

| 질문 의도 | 검색 범위 |
| --- | --- |
| 변경 이유·이력·비교: “현재 배포 금지가 바뀐 계기는?” | 과거 가이드와 장애 문서 유지 |
| 특정 버전: “현재 배포 가이드 v22의 금지 시간은?” | 명시한 v22 우선 |
| 현재 정책: “현재 프로덕션 배포 명령은?” | 배포 가이드의 최신 버전 |
| 오류·장애 질문, 여러 버전 또는 불분명한 질문 | 제한하지 않음 |

```text
“현재 프로덕션 배포 명령은 무엇인가요?”
         ↓ 배포 질문인지, 이력·특정 버전 요청인지 확인
현재 정책 질문 → DB의 가이드 버전을 숫자로 비교
         ↓ 가장 큰 버전 선택 (현재 v30)
v30의 청크만 대상으로 키워드·벡터 검색
         ↓ 문서별 대표 청크 선택
“현재 규칙” → deploy run --env prod --ticket <id>
```

Top 3를 뽑기 **전에** 양쪽 검색에 같은 조건을 적용한다. v30으로 범위를 좁히면 문서가 하나이므로 결과도 하나다. 최신 번호를 코드에 고정하거나 평가 정답을 검색 조건으로 쓰지 않는다. API·MCP·CLI에 같은 규칙을 적용했고, 문서 재색인은 필요하지 않다.

**같은 질문의 개선 결과:** keyword·vector·hybrid 모두 `deploy-guide-v30`의 `현재 규칙` 청크를 1위로 반환했고, 실제 배포 명령이 근거에 포함됐다.

**[전체 평가](https://github.com/hhk22/llm-wiki/blob/v2-search-quality/evaluation/results-v2-filters.json) — 고정 20문항의 문서 Hit@3**

| 검색 방식 | 필터 전 → 후 | 기존 성공 문항의 퇴보 |
| --- | --- | --- |
| 보정 키워드 | 2/20 → **8/20** | 없음 |
| 벡터 | 14/20 → **19/20** | 없음 |
| Hybrid | 11/20 → **15/20** | 없음 |

**벡터 + 필터 95% > hybrid + 필터 75%.** 최신 정책 5문항은 벡터 기준 정답 문서 0/5 → 5/5이며, 이는 **버전 선택 규칙의 효과**다. 필터 조건이 없는 교차 문서 질문의 실패는 그대로 남았다.

**추가 검증:** 평가 실행 전에 고정한 [별도 표현 6문항](https://github.com/hhk22/llm-wiki/blob/v2-search-quality/evaluation/filter-validation.yaml)에서도 벡터 Hit@3는 [3/6 → 5/6, 퇴보 없음](https://github.com/hhk22/llm-wiki/blob/v2-search-quality/evaluation/results-v2-filters-validation.json)이었다. DB 임시 테이블에서 v31·v100 추가 시 최신 선택이 바뀌고, v9 지정·없는 버전·잘못된 버전 메타데이터·이력 질문도 검증했다. 운영 데이터는 바꾸지 않았다.

**남은 문제:** 최신 정책 5문항 중 4개는 답이 담긴 `현재 규칙` 청크를 반환했지만, “변경 티켓 번호가 반드시 필요한가요?”는 v30의 `변경 없음` 요약 청크를 반환했다. **문서 적중 5/5가 답변 근거 확보 5/5는 아니다.** 이 청크 선택 문제와 교차 문서 누락을 4단계에서 살펴본다.

**적용 범위:** `배포`와 `현재·최신·지금·요즘·오늘·현행` 등 명시적 표현을 쓰는 한국어 질문에 대한 규칙이다. 암묵적인 최신 의도까지 이해하는 분류기는 아니다. 현재 `topic=deploy-guide`를 하나의 버전 묶음으로 사용하며, 여러 가이드를 다루려면 별도 묶음 ID가 필요하다.
...
문서 120개·청크 150개, 임베딩 모델과 768차원 설정을 유지한다. 아래는 hybrid와 재정렬을 비교할 실험 구조다. 현재 기본 경로는 벡터 + 버전 필터이며, hybrid를 더 좋은 검색 방식으로 전제하지 않는다.
...
이번 hybrid 실험은 `evaluation/results-v2-hybrid.json`에 기록했고 v1 결과는 보존했다. 필터 실험은 `evaluation/results-v2-filters.json`에 별도로 기록했다. 이후 재정렬 실험도 설정과 결과를 보존하고 전후 수치와 대표 성공·실패 사례를 추가한다.

v2의 완료 기준은 **어떤 변경이 어떤 질문을 개선했고, 어떤 실패가 남았는지 재현 가능한 기록으로 설명하는 것**이다. 그 결과를 보고 Wiki 생성·문서 간 연결 같은 다음 단계가 필요한지 판단한다.

v1 구현은 [기준선 브랜치](https://github.com/hhk22/llm-wiki/tree/docs/llm-wiki-devlog), v2 실험 기록과 이후 작업은 [검색 품질 개선 브랜치](https://github.com/hhk22/llm-wiki/tree/v2-search-quality)에서 확인할 수 있다. 현재 코드에는 오류 코드 조사 제거·hybrid 옵션·버전 필터가 반영됐으며, reranker는 아직 적용 전이다.
