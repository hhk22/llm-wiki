...
이 글은 v1의 앞부분인 실험용 문서 준비와 색인 과정을 다룬다. 검색·답변·평가는 다음 글에서 이어간다.
...
색인한 청크로 질문에 답하는 부분을 만든다. 3번의 평가 질문 20개로 키워드 검색과 벡터 검색을 같은 조건에서 실행하고, 단일 문서 질문은 Hit@1·Hit@3, 문서 간 연결 질문은 두 문서의 Top 3 포함 여부를 기록한다. 유형별로 어디서 맞고 어디서 틀리는지가 다음 개선의 근거가 된다.
