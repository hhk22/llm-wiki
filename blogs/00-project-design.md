...
v1 RAG 기준선을 완료했다. 다음은 같은 평가 질문으로 검색 품질을 개선하는 v2이며, 현재는 구현 전 계획 단계다. 이후에는 남은 실패를 근거로 Wiki 생성·지식 갱신·운영 기능의 필요성을 검증한다.
...
| **[v2 · 검색 품질 개선](https://github.com/hhk22/llm-wiki/blob/v2-search-quality/blogs/02-search-quality.md) (계획)** | 식별자 보정, hybrid, 문서 유형·최신 버전 필터의 효과를 분리해 비교한다. 남는 순위 오류에 reranker를 실험하고 품질·지연·비용으로 채택 여부를 판단한다. |
