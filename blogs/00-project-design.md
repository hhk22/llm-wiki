...
RAG 구축을 기준선으로 시작해 LLM Wiki로 전환하고, 지식 갱신과 운영 문제를 해결하며 서비스로 발전시키는 과정을 버전별로 기록한다.

| 버전 | 구현할 내용 |
| --- | --- |
| **v1 · RAG 구축** | 가상 사내 문서 100~200개를 수집·청킹·임베딩하고, 벡터 검색과 출처 기반 답변을 구현해 기준선을 만든다. |
| **v2 · LLM Wiki 전환** | RAG에서 드러난 반복 검색과 문서 간 종합의 한계를 바탕으로, LLM이 주제별 Wiki·목차·관련 링크·출처를 생성하는 구조로 전환한다. |
| **v3 · 지식 갱신** | 원본 문서 변경을 추적해 관련 Wiki를 갱신하고, 버전·변경 이력·상충 내용을 관리한다. |
| **v4 · 성능과 안정성** | 처리 병목과 동시성 오류를 재현하고, Worker·작업 상태·멱등성·재시도로 해결한다. |
| **v5 · 운영 피드백** | 질의 성공·실패, 응답 시간과 사용자 피드백을 관찰하고, 병목이 확인되면 Redis 캐시와 cache hit/miss를 측정한다. |
| **v6 · 서비스 완성** | 접근 제어, 운영 화면, Docker Compose 실행 환경, 자동 테스트와 배포·데모를 완성한다. |
